...
const SingleAssetModal = ({  setModalOpen ,setLandOwnerModal ,  onClosed, assetDetails,setAssetDetails, onInputChange , onEdit , updateAssetDetails , setIsEdit}) => {
...
const handleClosed = ()=>{
      ...
      <div className="bg-white p-8 rounded shadow-lg 2xl:w-[40vw] md:w-[55vw] xl:w-[42vw] relative">
        ...
        <div className="flex  flex-col mb-4">
          <div className="w-3/5 mb-2">
            <label className='md:text-[11px] xl:text-[12px] font-inter font-semibold'>Asset ID</label>
            <input
              type="text"
              name="assetId"
              value={assetDetails?.assetId || ''} // Control the input with assetDetails state
              onChange={onInputChange}
              className="border rounded-sm p-2 w-full mt-1 outline-none font-inter md:text-[11px] xl:text-[12px]"
              placeholder='type your asset Id'
            />
          </div>
          <div className="w-3/5 ">
            <label className='md:text-[11px] xl:text-[12px] font-semibold font-inter'>Rate in ₹/m³</label>
            <input
              type="number"
              name="rate"
        ...
        <div className="flex justify-between items-center mb-4">
          <span className="font-semibold md:text-[11px] xl:text-[12px] font-inter">Location</span>
        ...
        <div className="flex justify-between mb-4">
          <div className="w-1/3 md:text-[11px] xl:text-[12px] ">
            ...
            <input
              ...
              className="border outline-none border-custom-border bg-background-color3  rounded-sm p-1 py-2 w-full"
            />
          </div>
          ...
          <div className="w-1/3 ml-4 md:text-[11px] xl:text-[12px]">
            ...
            <input
              ...
              onChange={onInputChange}
              className="border outline-none  border-custom-border  rounded-sm p-1 py-2 w-full"
            />
          </div>
        </div>
       ...
       <div>  <label className='md:text-[11px] xl:text-[12px] mb-2 font-inter font-semibold '>Tahsil</label></div>
       <div className="flex  items-center mb-4">
          <div className="flex items-center">
            <span className='md:text-[11px] xl:text-[12px] md:w-16 font-inter font-semibold'>Soil Test</span>
            <Switch
              ...
              <span
                className={`${
                  assetDetails?.soilTest ? 'translate-x-3' : 'translate-x-1'
                } inline-block w-4 h-4 transform bg-white rounded-full transition-transform`}
              />
            </Switch>
          </div>
          <div className="flex items-center px-28">
            <span className='md:text-[11px] xl:text-[12px] md:w-16 xl:w-20  font-inter font-semibold'>Seed Order</span>
            <Switch
  ...
  <label className="md:text-[11px] xl:text-[12px] font-inter font-semibold">Operator</label>
  <input
    type="text"
    name="name"
    value={assetDetails?.name}
    onChange={handleInputChange}
    className="border p-1 w-full md:text-[11px] xl:text-[12px] py-2 outline-none rounded-sm px-1 pr-8" // Add padding to the right
    placeholder="Name"
  />
  <span className="absolute right-2 top-10 transform -translate-y-1/2">
  <svg width="15" height="8" viewBox="0 0 15 8" fill="none" xmlns="http://www.w3.org/2000/svg" className='mt-[2px] lg:mt-0 md:w-2 md:h-2 xl:h-3 xl:w-3'>
          ...
          <div className="w-1/2 ml-4">
            <label className="md:text-[11px] xl:text-[12px] font-inter font-semibold ">Phone Number</label>
            <input
              type="tel"
              name="phoneNumber"
              value={assetDetails?.phoneNumber }
              onChange={handleInputChange} // Use the existing onInputChange function
              className="border p-1 w-full md:text-[11px] xl:text-[12px] py-2 outline-none rounded-sm px-1"
        ...
        <button onClick={saveOperator} className="bg-btn-color text-text-color2  md:px-5 xl:px-6 py-2 rounded md:text-[11px] xl:text-[12px] font-inter  ">
         Next
        </button>
        </div>
